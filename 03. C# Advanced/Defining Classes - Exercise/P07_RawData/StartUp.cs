...
namespace P07_RawData
{
    public class StartUp
    {
        static void Main(string[] args)
        {
            ...
            {
                case "fragile":
                    Console.WriteLine(String.Join(Environment.NewLine,cars.Where(x => x.Cargo.Type == "fragile" && x.Tires.Any(t => t.Pressure <1))));
                    break;
                case "flammable":
                    Console.WriteLine(String.Join(Environment.NewLine,cars.Where(x => x.Cargo.Type == "flammable" && x.Engine.Power >250)));
                    break;
                default:
                    break;
            }
